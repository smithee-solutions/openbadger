...
{ /* main for obtest-challenge */
  ...
  OB_CONTEXT *ctx;
  unsigned char dummy_encrypted_payload [256];
  DWORD dwRecvLength;
  ...
  // in case no payload supplied.  fill the buffer with a constant.

  memset(dummy_encrypted_payload, 0x17, sizeof(dummy_encrypted_payload));
  memcpy(ctx->challenge_message, dummy_encrypted_payload, sizeof(ctx->challenge_message));
  ...
  if (status EQUALS ST_OK)
  {
    ...
    // we are set up to talk to the card. perform the challenge.
