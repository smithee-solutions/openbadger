...
{ /* main for divutil */
  ...
  while (!done)
  {
    ...
    case OB_SELFTEST:
      ...
      memcpy(ctx->secret_key, &secret_key_default, OB_AES128_KEY_SIZE);
      status = ST_OK;
      selftest = 1;
      break;
  ...
  if (status EQUALS ST_OK)
  {
    if (selftest)
    {
      comparison = memcmp(expected_K0, K0, OB_AES128_KEY_SIZE);
      if (ctx->verbosity > 9)
  ...
  if (status EQUALS ST_OK)
  {
    ...
    if (ctx->verbosity > 3)
      fprintf(LOG, "%s", ob_buffer_dump_string(ctx, K1, OB_AES128_KEY_SIZE, "K1 input to step 2:"));
    /*
